<template>
  <section class="bl-services2">
    <div class="ln-container">
      <h2 class="ln-base-title-lg-bold">Услуги</h2>
      <div class="bl-services2-slider" v-if="dataServices.length > 0">
        <agile
          :nav-buttons="false"
          :dots="false"
          ref="service2slider"
          ...
          :infinite="infinite"
          :options="{responsive:[{breakpoint: 992,settings: {slidesToShow: 2}},{breakpoint: 768,settings: {slidesToShow: 2}}, {breakpoint: 500,settings: {slidesToShow: 1}}]}"
          >
            <div class="bl-services2-slide" v-for="service in dataServices">
              <div class="bl-services2-item">
                <h3 class="bl-services2-title">{{ service.name }}</h3>
                <div class="bl-services2-subtitle" v-if="service.short_description" v-html="service.short_description"></div>
                <div class="bl-services2-area" v-if="service.description">
                  <div class="bl-services2-text" v-bar>
                    <div>
                      <div class="bl-services2-scroll" v-html="service.description"></div>
                    </div>
                  </div>
                </div>
                <div class="bl-services2-thumb"
                  :style="{'background-image':'url(' + service.fm_image + ')'}"
                ></div>
              </div>
            </div>
        </agile>

            <div class="bl-services2-btn bl-services2-prev" @click="$refs.service2slider.goToPrev();" v-if="slidesToShow >= 3">
            ...
            <div class="bl-services2-btn bl-services2-next" @click="$refs.service2slider.goToNext();" v-if="slidesToShow >= 3">
              <svg width="14" height="12" viewBox="0 0 14 12" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div v-else >
        <div class="bl-empty-text">
          Не создано ни одной услуги.<br>
          Добавить услуги вы можете в <a href="/account/service" target="_blank">личном кабинете</a>
        </div>
      </div>
    </div>
  </section>
...
export default {
  ...
  computed: {
    ...
    slidesToShow(){
      if(this.dataServices.length >= 3) {
        return 3;
      }
      return this.dataServices.length;
    },
    infinite() {
      if(this.dataServices.length >= 3) {
        return true;
